    ...
    def get_candidate(self, search):
        try:
            ...
            if response.status_code == 200:
                ...
                search_marvel_srp_div = soup.find('div', class_='search-marvel-srp')

                # Get the HTML content of the div
                if search_marvel_srp_div:
                    html_content = search_marvel_srp_div.prettify()
                    print(html_content)
                else:
                    print("Div with class 'search-marvel-srp' not found.")
            else:
                print(f"Failed to fetch the page. Status code: {response.status_code}")
        except Exception as e:
            print("Exception in get_candidate: ", e)

    def get_candidate_email_and_phone(self, profile_id):
        try:
            res = self._fetch(f"/graphql?includeWebMetadata=true&variables=(memberIdentity:{profile_id})&queryId=voyagerIdentityDashProfiles.c7452e58fa37646d09dae4920fc5b4b9")
            ...
            for profile in includes:
                if profile["$type"] == "com.linkedin.voyager.dash.identity.profile.Profile":
                    data = profile
                    break
            ...
            for phone in data["phoneNumbers"]:
                phone_numbers.append(phone["phoneNumber"]["number"])

            return {
                "birth_date" : data["birthDateOn"],
            ...
            print("Exception in get_candidate_email_and_phone: ", e)